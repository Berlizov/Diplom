...
/**
 * Created by Eugene Berlizov on 20.09.2014.
 */
    ...
    public static void createDefTables() {
        try {
            ...
            sql += "CREATE TABLE PROJECT_USER  " +
                    ...
                    " DT            TIMESTAMP   DEFAULT CURRENT_TIMESTAMP, " +
                    " UNIQUE(PROJECT , USER) ON CONFLICT REPLACE);\n";
            sql += "CREATE TABLE TASK " +
                    ...
                    " SPRINT_ID     INTEGER, " +
                    " DT            TIMESTAMP   DEFAULT CURRENT_TIMESTAMP, " +
                    " UNIQUE(PROJECT , NAME ) ON CONFLICT FAIL);\n";
            sql += "CREATE TABLE TASK_DEVELOPER " +
                    "(ID            INTEGER     PRIMARY KEY AUTOINCREMENT," +
                    " TASK_ID       INTEGER     NOT NULL, " +
                    " DEVELOPER     CHAR(50)    NOT NULL, " +
                    " DT            TIMESTAMP   DEFAULT CURRENT_TIMESTAMP, " +
                    " UNIQUE(TASK_ID , DEVELOPER) ON CONFLICT REPLACE);\n";
            ...
            sql += "CREATE TABLE SUBTASK " +
                    ...
                    " DT            TIMESTAMP   DEFAULT CURRENT_TIMESTAMP, " +
                    " UNIQUE(TASK_ID, NAME) ON CONFLICT FAIL);\n";
            ...
            addUser(new User("developer", "developer", UsersTypes.DEVELOPER));
            addUser(new User("product owner", "product owner", UsersTypes.PRODUCT_OWNER));
            addUser(new User("stakeholder", "stakeholder", UsersTypes.STAKEHOLDERS));
    ...
    public static boolean addUser(User u) {
        try {
            String sql = "INSERT INTO USERS (LOGIN, PASS, TYPE) " +
                    "VALUES ( '" + u.getLogin() + "', '" + u.getPass() + "', " + u.getType().getId() + " );";
            stmt.executeUpdate(sql);
        ...
        try {
            ...
            switch (user.getType()) {
                ...
                case PRODUCT_OWNER:
                    ...
                    break;
                default:
                    sql ="SELECT * FROM PROJECT INNER JOIN PROJECT_USER\n" +
                            "ON PROJECT.NAME=PROJECT_USER.PROJECT\n" +
                            "WHERE PROJECT_USER.USER='"+ user.getLogin() + "'";
                    break;
            }
            System.out.println(sql);
            ResultSet rs = stmt.executeQuery(sql);
 ...
 * */
    public static boolean addTask(Task task) {
        try {
            String sql = "INSERT INTO TASK (NAME, PROJECT, COMPLEXITY, DESCRIPTION) " +
    ...
    public static Task[] getProjectUserTasksComplexity(String project, User user) {
        try {
            Task[] tasks = getProjectTasks(project);
            for (Task task : tasks) {
                task.setUserComplexity(PokerCardDeck.NOTSET);
            }
            String sql = "SELECT TASK.NAME," +
                    "PROJECT," +
                    "TASK_DEVELOPER_COMPLEXITY.COMPLEXITY AS USER_COMPLEXITY " +
                    "FROM TASK INNER JOIN TASK_DEVELOPER_COMPLEXITY " +
                    "ON TASK.ID=TASK_DEVELOPER_COMPLEXITY.TASK_ID " +
                    "WHERE TASK_DEVELOPER_COMPLEXITY.DEVELOPER='"+user.getLogin()+"' " +
            ...
            while (rs.next()) {
                for (Task task : tasks) {
                    if (task.getName().equals(rs.getString("NAME"))) {
                        task.setUserComplexity(PokerCardDeck.valueOf(rs.getInt("USER_COMPLEXITY")));
                    }
                }
            }
            return tasks;
    ...
    public static boolean setTaskComplexity(Task task, User user) {
        try {
            String sql;
            switch (user.getType()){
                case PRODUCT_OWNER:
                    ...
                    break;
                case DEVELOPER:
                    sql = "INSERT INTO TASK_DEVELOPER_COMPLEXITY" +
                            "(TASK_ID, DEVELOPER, COMPLEXITY,DT)" +
                            " values (" +
                            "(SELECT ID FROM TASK WHERE NAME='" +task.getName()+"' AND PROJECT = '"+task.getProject()+"'), "+
                            " '"+user.getLogin()+"', "+task.getUserComplexity().ordinal()+", CURRENT_TIMESTAMP);";
                    break;
    ...
    public static boolean addSubtask(Subtask subtask) {
        try {
            String sql = "INSERT INTO SUBTASK (NAME, TASK_ID, COMPLETENESS, DT) " +
                    "VALUES ('"+subtask.getName()+"', " +
                    "(SELECT ID FROM TASK WHERE NAME='"+subtask.getTaskname()+"' " +
                    "AND PROJECT='"+subtask.getProject()+"'), '"+subtask.getFinish()+"', CURRENT_TIMESTAMP)";
            System.out.println(sql);
            stmt.executeUpdate(sql);
            return true;
        } catch (Exception e) {
    ...
    public static Subtask[] getSubtasks(Task task) {
        try {
            String sql = "SELECT SUBTASK.NAME, SUBTASK.COMPLETENESS FROM SUBTASK INNER JOIN TASK " +
                    "ON TASK.ID=SUBTASK.TASK_ID " +
                    "WHERE TASK.NAME='"+task.getName()+"' " +
                    "AND TASK.PROJECT='"+task.getProject()+"'";
            ArrayList<Subtask> subtasks=new ArrayList<>();
            System.out.println(sql);
            ResultSet rs = stmt.executeQuery(sql);
            while (rs.next()) {
                subtasks.add(new Subtask(task.getName(),task.getProject(),rs.getString("NAME"),Boolean.parseBoolean(rs.getString("COMPLETENESS"))));
                System.out.println(subtasks.get(subtasks.size() - 1));
            }
    ...
    public static boolean changeSubtaskCompleteness(Subtask subtask) {
        try {
            String sql = "UPDATE SUBTASK " +
                    "SET COMPLETENESS='"+subtask.getFinish()+"', DT = CURRENT_TIMESTAMP "+
                    "WHERE NAME='"+subtask.getName()+"' AND " +
                    "TASK_ID =(SELECT ID FROM TASK WHERE NAME='"+subtask.getTaskname()+"' AND PROJECT='"+subtask.getProject()+"')";
    ...
    public static Boolean addTaskDescription(Task task,String str) {
        try {
    ...
    public static Boolean changeTaskUsers(Task task, String[] users) {
        try {
            int index=-1;
            ...
            if(index!=-1) {
                sql = "DELETE FROM TASK_DEVELOPER\n" +
                        "WHERE TASK_ID=" + index + "";
                stmt.executeUpdate(sql);
            }
            if(users.length>0) {
                sql = "INSERT INTO TASK_DEVELOPER (TASK_ID,DEVELOPER)\n" +
                        "VALUES\n";
                for (int i = 0; i < users.length; i++) {
                    if (i != 0)
                        sql += ", ";
    ...
    public static User[] getTaskUsers(Task task) {
        try {

            String sql = "SELECT USERS.LOGIN, USERS.TYPE FROM USERS INNER JOIN TASK_DEVELOPER " +
                    "ON USERS.LOGIN = TASK_DEVELOPER.DEVELOPER " +
                    "INNER JOIN TASK " +
                    "ON TASK_DEVELOPER.TASK_ID = TASK.ID " +
                    "WHERE NAME = '" + task.getName() + "' and PROJECT = '" + task.getProject() + "' " +
                    "ORDER BY USERS.TYPE DESC, USERS.LOGIN ASC ";
            System.out.println(sql);
            ResultSet rs = stmt.executeQuery(sql);
  ...
  //  public static Task[] getTasksBySprint(String project, Sprint sprint){
